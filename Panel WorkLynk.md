...
Facturas (desde `748dce8`): el PDF es una factura completa de autónomo según el RD 1619/2012. El emisor es la persona física y WORKLYNK aparece como nombre comercial. Lleva IVA y retención de IRPF, y `invoice_totals.total` es lo que paga el cliente. Requiere la migración `202609240014_invoice_legal.sql`. Desde `023822a` (migración `202609240015_fiscal.sql`): borrador sin número → «Emitir» (controles legales en cliente y servidor, número `AAAA-NNN` o `RAAAA-NNN`, datos e importes congelados) → cobro. Los triggers impiden modificar o borrar facturas emitidas. Pestañas Facturas · Impuestos · Gastos · Datos fiscales en `/invoices`. Lógica y normativa en `src/lib/fiscal.ts`, con pruebas. Pendiente: Veri*factu (obligatorio el 1-7-2027), editar gastos y festivos en el calendario.

Presupuestos (desde `2334df4`, migración `202609240016_quotes.sql`): serie `PAAAA-NNN`, independiente de las facturas `AAAA-NNN` y `RAAAA-NNN`. Pueden emitirse antes del alta en Hacienda. Una vez emitidos no se modifican (se duplican). `quote_to_invoice` crea borradores de factura con `invoices.quote_id`. Rutas: `/invoices/quotes`.

Gastos (migración `202609240017_recurring_expenses.sql`): suscripciones en `recurring_expenses`, que al abrir la pestaña generan un gasto por cargo (índice único por suscripción y fecha). `expenses.reverse_charge` marca las compras a proveedores extranjeros sin IVA: su IVA se autoliquida y compensa en el 303. Interfaz en `src/pages/ExpensesPanel.tsx`.

Descuentos (migración `202609240018_discounts_no_withholding.sql`): `discount_pct` por línea en `invoice_items` y `quote_items`. La base es la suma de cantidad × precio × (1 − dto.), redondeada una vez, igual en SQL que en `src/lib/invoice.ts`.

Tarifas (migración `202609240019_pricing.sql`): la tabla `pricing` guarda una sola fila con la tarifa en JSON. La lógica está en `src/lib/pricing.ts` (con pruebas) y la pantalla en `src/pages/Pricing.tsx`. Mientras no haya nada guardado se usa `PRICING_PROPOSAL`. «Crear presupuesto» llama a `save_quote_with_items`: las licencias anuales entran como línea del primer año y las cuotas mensuales van en las condiciones.

Historial de cliente (migración `202609240020_client_notes.sql`): `src/components/ClientHistory.tsx` junta presupuestos (con condiciones), facturas y `client_notes`. Las notas solo se añaden: RLS permite leer e insertar, no editar ni borrar. El buscador está en `src/lib/search.ts`, no distingue acentos ni mayúsculas.
