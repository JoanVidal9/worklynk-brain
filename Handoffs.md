...
- Cuarta pasada, página a página (orden: KERN → Refórmalo → Servicios → Proceso → Casos + caso → Estudio → Contacto). KERN hecho (`6ef3718`): sin la cifra de documentos al mes, sin vocabulario de "plantilla Word" ni "copiar y pegar", mismo sistema que la portada y maqueta con aprobación por fases. El caso de ingeniería aún cita "veinte al mes": revisarlo al llegar a Casos.
